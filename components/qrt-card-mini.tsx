...
interface QRTCardMiniProps {
  /** The user's QR ID data or null if not available */
  qrtId: QRTIDRequest | null
  /** Optional callback when the "Request QR ID" CTA is clicked */
  onRequestClick?: () => void
}
 ...
 *
 * A compact, reusable QR ID card display component for the dashboard.
 * Features smooth 3D flip animation and responsive design.
 *
 * Features:
 * - Empty state CTA when no QR ID exists (gradient card with call-to-action)
...
export function QRTCardMini({ qrtId, onRequestClick }: QRTCardMiniProps) {
  ...
  const handleCardClick = () => {
    ...
    // Navigate to full QR ID details page
    ...
    return (
      ...
      >
        <div className="flex items-center justify-between">
          <div>
            <p className="text-white/80 text-sm font-medium mb-1">QR ID</p>
        ...
        <p className="text-white/60 text-xs mt-3">
          Get your official Barangay QR IDentification card
        </p>
      </div>
    )
  }

  // Has QR ID - Show mini card with flip functionality
  return (
    <div className="relative max-w-md mx-auto">
      <div
        ...
        role="button"
        tabIndex={0}
        aria-label={`View QR ID details for ${qrtId.fullName}`}
        onKeyDown={(e) => {
          if (e.key === 'Enter' || e.key === ' ') {
            e.preventDefault()
            handleCardClick()
          ...
          >
            {qrtId.idBackImageUrl ? (
              <Image
                ...
                alt={`QR ID Back for ${qrtId.fullName}`}
                fill
                className="object-cover"
                priority
              />
      ...
      <div className="mt-3 flex items-center justify-between">
        <div>
          <p className="text-xs text-gray-500">QR ID Card</p>

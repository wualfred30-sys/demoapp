...
import { IDInspectionModal } from "@/components/dashboard/id-card/id-inspection-modal"
...
export default function DashboardPage() {
  ...
  // useQRT: Provides user's QR ID card data
  ...
  // Load user's QR ID (from useQRT context)
  // Edge cases handled:
  // 1. User with no QR ID → setUserQrtId(null) → CTA shows
  // 2. User with pending/processing QR ID → Card with processing state
  // 3. User with issued QR ID → Card with images shows
  // 4. User with multiple QR IDs → Correct one prioritized (issued/ready first)
  useEffect(() => {
  ...
  // Comment 4 fix: compute once per render from the stable id guaranteed by auth-context.
  // user.id is always present because login() in auth-context assigns it.
  // The object is never re-generated across calls because user is stable after auth.
  const residentBase = {
    id: user.id!,
      ...
      <main className="flex-1 px-4 pb-24 pt-6">
        ...
        {user && (
          <section className="mb-5">
            <h2 className="text-[18px] font-bold text-[#111827] mb-3 px-1">My Digital ID</h2>
            <IDCard
              resident={residentBase}
              layoutId={`id-card-${user.id!}`}
        ...
        <section className="mb-6">
          <h2 className="text-[18px] font-bold text-[#111827] mb-2 px-1">Recent Activity</h2>
          <ActivityFeed />
        </section>

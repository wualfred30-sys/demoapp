...
export default function QrtIdDetailPage() {
  ...
  const handleShare = async () => {
    ...
    const shareData = {
      title: `QR ID - ${qrtId.fullName}`,
      text: `Verify my Barangay Mawaque QR ID: ${qrtId.qrtCode}`,
      url: url,
    }
  ...
  const handlePrint = () => {
    ...
    printWindow.document.write(`
      ...
      <html>
        <head>
          <title>QR ID - ${qrtId.fullName}</title>
          <style>
            @media print {
        ...
        <body>
          <h1>QR ID Card - ${qrtId.qrtCode}</h1>
  ...
  if (!qrtId) {
    return (
      <div className="flex min-h-screen flex-col items-center justify-center bg-[#F9FAFB] p-6 text-center">
        ...
        <h2 className="text-xl font-black text-gray-900 mb-2">QR ID Not Found</h2>
        <p className="text-gray-500 max-w-sm mb-6">
          The QR ID you are looking for might have been moved or doesn't exist.
        </p>
        <Button onClick={() => router.push("/requests?tab=qrt")} className="h-12 w-full rounded-2xl bg-emerald-600 font-bold">
          Back to Requests
        </Button>
      </div>
    )
  ...
  return (
    <div className="flex min-h-screen flex-col bg-[#F9FAFB] pb-24 pt-12 px-4 max-w-none">
      ...
      <header className="fixed top-0 left-0 right-0 z-30 flex h-16 items-center bg-white/80 backdrop-blur-xl px-4 border-b border-gray-100">
        <button
          onClick={() => router.push("/requests?tab=qrt")}

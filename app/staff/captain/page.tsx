...
export default function CaptainDashboard() {
  ...
  const handleQRScan = async (data: string) => {
    try {
      ...
      let qrt: QRTIDRequest | undefined | null

      if (verificationCode) {
        console.log(`[Scanner] Scanned verificationCode: ${verificationCode}`)
        qrt = await findQRTByVerificationCode(verificationCode)
        ...
        setScanError("Invalid QR code: No verification code or QR Code found")
        return
      }

      if (qrt && verificationCode) {
        setScannedQRT(qrt)
        await logVerification(qrt.qrtCode, verificationCode, staffUser?.fullName || "Captain")
        ...
        setScanError("QR ID not found in system")
          ...
          >
            <CardContent className="flex items-center justify-between p-3">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm font-semibold text-gray-900">Quick Scan QR ID</p>
              ...
              {certificates.slice(0, 3).map((cert) => (
                <div key={cert.id} className="flex items-center justify-between rounded-lg bg-slate-50 p-2.5">
                  <div className="flex items-center gap-2">
                    <div
                      className={`flex h-7 w-7 items-center justify-center rounded-full ${cert.status === "ready" ? "bg-emerald-100" : "bg-amber-100"
                        }`}
                    >
                      {cert.status === "ready" ? (
                  ...
                  <span
                    className={`rounded-full px-2 py-0.5 text-[10px] font-medium ${cert.status === "ready" ? "bg-emerald-100 text-emerald-700" : "bg-amber-100 text-amber-700"
                      }`}
                  >
                    {cert.status}
                  </span>
